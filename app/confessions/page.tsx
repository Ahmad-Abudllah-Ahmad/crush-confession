...
export default function ConfessionsPage() {
  ...
  // Check authentication and fetch confessions
  useEffect(() => {
    ...
    if (status === 'unauthenticated') {
      ...
      console.log('User is authenticated, fetching confessions')
  ...
  const fetchConfessions = async () => {
    ...
    try {
      ...
      console.log('Fetched confessions:', data.confessions.length)
  ...
  const fetchMyConfessions = async () => {
    ...
    try {
      ...
      setError(err.message || 'An error occurred while fetching your confessions')
      console.error(err)
    } finally {
      setIsLoading(false)
    }
  ...
  const applyFilter = (filterType: FilterType) => {
    ...
    switch (filterType) {
      ...
      case 'mine':
        ...
        filtered = filtered.filter(confession => confession.isSender === true)
        break
  ...
  const handleRevealInterest = async (confessionId: string) => {
    try {
      ...
      alert('Interest revealed successfully! If they also reveal interest, you can start chatting.');
    } catch (err: any) {
  ...
  const handleDelete = async (confessionId: string) => {
    ...
    try {
      ...
      const updatedConfessions = confessions.filter(confession => confession.id !== confessionId)
      setConfessions(updatedConfessions)
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="flex flex-wrap justify-between items-center mb-8">
          <h1 className="text-2xl font-bold text-purple-800 mb-2 md:mb-0">Campus Confessions</h1>
          <NewConfessionButton />
        </div>
        ...
        {error && (
          <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
            ...
            <button 
              ...
              onClick={() => activeFilter === 'mine' ? fetchMyConfessions() : fetchConfessions()}
            >
              Try Again
            </button>
          </div>
